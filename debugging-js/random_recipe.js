/*
The "random" function has a return value of an array. When initializing 
dishName, the program concatenates several arrays. JavaScript tries to convert 
the arrays to strings (using the values inside), and concatenates them to a 
single string. Thus, the function fails to use the method .join(' ') on the 
variable "dishName". A solution to this runtime error would be to join the 
three arrays with .concat(), enabling the program to use .join() on the 
resulting array. 
*/
